...
def parse_int_list(csv_string):
    ...
    Handles None and empty strings gracefully.
    Returns empty list for strings that don't contain valid integers.
    """
    if csv_string is None:
        return []
    if not isinstance(csv_string, str):
        raise TypeError(f"Expected string, got {type(csv_string).__name__}")
    ...
    for p in parts:
        stripped = p.strip()
        if stripped:  # Only process non-empty parts
            try:
                result.append(int(stripped))
            except ValueError:
                # Skip parts that cannot be converted to integers
                pass
    return result
